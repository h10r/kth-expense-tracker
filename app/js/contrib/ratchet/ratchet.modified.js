...
!function () {
  var findModals = function (target) {
    ...
    // add a triggeredActiveModal class, so we know which anchor
    // has triggered the current active modal 
    if (target.className.indexOf('open-modal') !== -1) {
      target.classList.add('triggered-active-modal');
    }
    // remove it when modal was closed again
    if (target.className.indexOf('close-modal') !== -1) {
      clickedEl = document.getElementsByClassName('triggered-active-modal');
      clickedEl[0].classList.remove('triggered-active-modal');
    }

    //console.log(target);
    //console.log(target.className);

    // added a.open-modal and a.close-modal to selector so it doesn't interfere
    // with angular.js routing anymore
    var i, modals = document.querySelectorAll('a.open-modal, a.close-modal');
